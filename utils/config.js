...
//		dojox/app/utils/config
	...
	return {
		...
		configProcessHas: function (/*Object*/ source) {
			...
			//		If the has test is true it will call configMerge to merge has sections back into the source
			// 		config.
			//		It will always remove the has section from the source after processing it.
			//		The names in the has section can be separated by a comma, indicating that any of those being true
			...
			for (var name in source) {
				...
				if (name === "has") { // found a "has" section in source
					for (var hasname in sval) { // get the hasnames from the has section
						if (!(hasname.charAt(0) === "_" && hasname.charAt(1) === "_") && sval &&
							typeof sval === "object") {
							...
							if (parts.length > 0) {
								while (parts.length > 0) {
									...
									if ((has(haspart)) || (haspart.charAt(0) === "!" && !(has(haspart.substring(1))))) {
										// if true this one should be merged
		...
		configMerge: function (/*Object*/ target, /*Object*/ source) {
			...
			//		anything starting with __ will be skipped and if the target is an array the source items will be
			// 		pushed into the target.
			// target:
