	...
	function (lang, declare, topic, on, Controller, hash) {
		...
		return declare(Controller, {
			...
			onStartTransition: function (evt) {
				...
				// Use _addToHistoryStack flag to indicate the _onHashChange method should add this hash to history
				// stack.  When add hash to history stack, this flag should be set to false, we do this in _addHistory.
			...
			_onHashChange: function (currentHash) {
				...
				if (this._index < 0 || this._index > (window.history.length - 1)) {
					throw new Error("Application history out of management.");
				}
				...
				if (this._oldHistoryLen > this._newHistoryLen) {
					//console.log("need to refresh _historyStack, oldLen:"+this._oldHistoryLen+", newLen: "+
					// this._newHistoryLen+", diff:"+this._historyDiff);
					this._historyStack.splice((this._newHistoryLen - this._historyDiff - 1),
						(this._historyStack.length - 1));

					// Reset _historyLen to make sure this._historyLen<window.history.length, so it will push this
					// hash to history stack.
				...
				// this._oldHistoryLen === this._newHistoryLen, it maybe need to refresh history stack or do history
				// go, back and forward, so we use _addToHistoryStack to identify the refresh operation.
				...
				if (this._historyLen < window.history.length) {
					...
					if (currentHash === this._current) {
						...
						//search in "forward"
						if (-1 === index) {
							for (var j = this._index; j < this._historyStack.length; j++) {
								if (currentHash === this._historyStack[j].hash) {
									index = j;
									break;
								}
							}
			...
			_go: function (index, step) {
				if (index < 0 || (index > window.history.length - 1)) {
					throw new Error("Application history.go steps out of management, index: " + index + " length: " +
				...
				topic.publish("/app/history/go",
					{"viewId": target, "step": step, "detail": this._historyStack[index].detail});

				// transition to the target view
				this.app.emit("app-transition", {
					viewId: target,
					opts: lang.mixin({reverse: (step <= 0)}, this._historyStack[index].detail,
						{"params": hash.getParams(this._current)})
				});
			}
		});
	});
