define(["dcl/dcl", "dojo/_base/lang", "dojo/_base/array", "dojo/_base/window",
	...
	function (dcl, lang, array, win, query, domGeom, domAttr, domStyle, registry, LayoutBase, layout,
		...
		// summary:
		//		Extends LayoutBase which binds "app-initLayout", "app-layoutView" and "app-resize" events on
		...
		return dcl(LayoutBase, {
			...
			resizeSelectedChildren: function (w) {
				for (var hash in w.selectedChildren) {	// need this to handle all selectedChildren
					if (w.selectedChildren[hash] && w.selectedChildren[hash].domNode) {
						this.app.log("in Layout resizeSelectedChildren calling resizeSelectedChildren calling " +
							"_doResize for w.selectedChildren[hash].id=" + w.selectedChildren[hash].id);
			...
			_doResize: dcl.superCall(function (sup) {
				...
				//		view instance needs to do layout.
					...
					if (!node) {
						this.app.log("Warning - View has not been loaded, in Layout _doResize view.domNode is not " +
							"set for view.id=" + view.id + " view=", view);
						return;
					}
					...
					var mb = {};
					if (!("h" in mb) || !("w" in mb)) {
						// just use dojo/_base/html.marginBox() to fill in missing values
			...
			layoutView: dcl.superCall(function (sup) {
			    return function (event) {
					// summary:
					...
					if (event.view) {
						//this.inherited(arguments);
						sup.call(this, event);
						// normally when called from transition doResize will be false, and the resize will only be done
						// when the app-resize event is fired
						if (event.doResize) {
