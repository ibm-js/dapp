	...
	function (lang, declare, has, win, config, domAttr, topic, domStyle, constraints, Controller) {
		...
		return declare(Controller, {
			...
			layoutView: function (event) {
				...
				if (event.removeView) {
					// if this view is being removed set display to none and the selectedChildren entry to null
					...
					if (parentSelChild) {
						...
						if (event.transition === "none" || event.currentLastSubChildMatch !== parentSelChild) {
							// only call hideView for transition none or when the transition will not hide it
